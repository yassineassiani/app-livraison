...
class Homcc extends StatefulWidget {
  const Homcc({super.key});
  

  @override
  State<Homcc> createState() => _HomccState();
}

class _HomccState extends State<Homcc> {
  String? nom ;
  String? photo;
  List supp=[
    "Food",
  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(
      ...
      builder: (context, snapshot) {
           ...
           InkWell(
            ...
            child: Column(
              ...
              children: [Container(
                ...
                child:const Icon(Icons.home_filled,color: Color.fromARGB(255, 198, 151, 105),
                size:35),
              ...
              Text("Call the Delivery     ",style:TextStyle(color:Colors.white54),)],
            ),
            ),
          )
       ...
       ,),SizedBox(height: 15,)
       ,Padding(padding:EdgeInsets.only(left:15),  child:Text("Popular Delivery Person",style:TextStyle(
        fontSize: 23,
        fontWeight: FontWeight.w500,
        color: Colors.black54
...
print(users.length);
        return GridView.builder(gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2), itemCount: users.length
       , shrinkWrap: true,
       itemBuilder: (context,index){
        return InkWell(
onTap: (){
   Get.to(Ded(), arguments:users [index].id);
...
child: Column(
  children: [
    ...
    mainAxisAlignment: MainAxisAlignment.center,
      ...
      children: [Icon(Icons.star,color:Colors.amber),Icon(Icons.star,color:Colors.amber),Icon(Icons.star,color:Colors.amber)]
    )
